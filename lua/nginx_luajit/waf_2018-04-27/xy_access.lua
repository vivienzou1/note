...
-- 获取所有参数的内容
	...
	if method == "POST" and http_content_type then
		...
		local from,to = string.find(http_content_type,"multipart/form-data; boundary=",1,true)
		if from then
...
local function action_deny(_error_message)
	...
	if config_base.denyMsg.state == "on" then
		...
		elseif tp_denymsg == "string" then
			...
			ngx.say(host_deny_msg.deny_msg.."\t".._error_message)
			ngx.exit(200)
		end
	end
	if type(config_base.denyMsg.msg) == "number" then
		ngx.exit(config_base.denyMsg.msg.."\t".._error_message)
	else
		ngx.header.content_type = "text/html"
...
if Xy_CCDeny == "on" then
    ...
    if req then
        if req > CCcount then
			action_deny("CC browse upper limit")
            --ngx.exit(503)
            --return true
        else
             limit:incr(token,1)
...
if Xy_CCDeny_URL == "on" then
	local uri = ngx.var.uri
	CCcount = tonumber(string.match(Xy_CCrate_URL,'(.*)/'))
	CCseconds = tonumber(string.match(Xy_CCrate_URL,'/(.*)'))
	local token = ip..uri
	local limit = ngx.shared.limit
	local req,_= limit:get(token)
	if req then
	    if req > CCcount then
	    	action_deny("CC URL browse upper limit")
		...
		if v.state =="on" and host_uri_remath(v.hostname,v.uri) then
			...
			if ip_count == nil then
				...
				if ip_count >= maxReqs then
					...
					action_deny("browse to the upper limit")
					action_deny("browse upper limit")

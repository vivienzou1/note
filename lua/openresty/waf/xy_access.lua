...
-- cc开关及参数
local tb_base_mod = getDict_Config("base")

-- 设置cc攻击频率(与url无关)，ptime:秒,cnt:次数--默认1分钟同一个IP只能请求100次
local xy_cc_deny_state 	= tb_base_mod.cc_deny.state
local xy_cc_deny_cnt 	= tb_base_mod.cc_deny.cnt
local xy_cc_deny_ptime 	= tb_base_mod.cc_deny.ptime

-- 设置cc攻击频率(与url有关)，ptime:秒,cnt:次数--默认1分钟同一个IP只能请求100次
local xy_cc_url_deny_state 	= tb_base_mod.cc_url_deny.state
local xy_cc_url_deny_cnt 	= tb_base_mod.cc_url_deny.cnt
local xy_cc_url_deny_ptime 	= tb_base_mod.cc_url_deny.ptime
...
local action_tag = ""
local function action_deny(_error_message)
	action_tag = "deny"
	_error_message = _error_message or tostring(_error_message)
	...
	if config_base.denyMsg.state == "on" then
		...
		if tp_denymsg == "number" then
			--ngx.exit(host_deny_msg.deny_msg.."\t".._error_message)
	...
	if type(config_base.denyMsg.msg) == "number" then
		...
		ngx.header.content_type = "text/html"
		-- ngx.say(tostring(config_base.denyMsg.msg.."\t".._error_message))
		ngx.say(waf_show_error_html)
		ngx.exit(200)
	end
end
...
-- host_Mod 规则过滤
-- 动作支持 （allow deny log）
if  config_is_on("host_Mod") and host_Mod_state == "on" and action_tag == "" then
...
--- SETP 13_01
-- CC攻击(针对所有请求，以IP为单位， 不区分具体访问的url)
if xy_cc_deny_state == "on" then
	local uri = ngx.var.uri
    -- CCcount = tonumber(string.match(Xy_CCrate,'(.*)/'))
    -- CCseconds = tonumber(string.match(Xy_CCrate,'/(.*)'))
    CCcount = tonumber(xy_cc_deny_cnt)
    CCseconds = tonumber(xy_cc_deny_ptime)
    local token = ip
    local limit = ngx.shared.limit
    local req,_ = limit:get(token)
    if req then
        if req > CCcount then
        	next_ctx.waf_log = "[CC_Mod] deny,CC:"..CCcount.."/"..CCseconds
			action_deny()
            --ngx.exit(503)
            --return true
        else
...
--- SETP 13_02
-- CC攻击(针对所有请求，以IP+URL为单位，区分具体访问的url)
if xy_cc_url_deny_state == "on" then
	local uri = ngx.var.uri
	CCcount = tonumber(xy_cc_url_deny_cnt)
	CCseconds = tonumber(xy_cc_url_deny_ptime)
	local token = ip..uri
	local limit = ngx.shared.limit
	local req,_= limit:get(token)
	if req then
	    if req > CCcount then
	    	next_ctx.waf_log = "[CC_URL_Mod] deny,CC:"..CCcount.."/"..CCseconds..",toekn:"..token
	    	action_deny()
	         --ngx.exit(sy_error_code_503)
	        --return true
	    else
	         limit:incr(token,1)
	    end
